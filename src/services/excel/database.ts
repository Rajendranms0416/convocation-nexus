...
export const saveTeacherData = async (data: Record<string, string>[], session?: string, tableName?: string): Promise<Record<string, string>[]> => {
  ...
  try {
    ...
    try {
      ...
      const dbRecords: DynamicTableRow[] = enhancedData.map(teacher => ({
      ...
      if (tableName) {
        ...
        for (let i = 0; i < dbRecords.length; i += batchSize) {
          ...
          try {
            ...
            const { error } = await queryDynamicTable(tableName)
              .insert(batch);
            ...
            for (const record of batch) {
              try {
                const { error: individualError } = await queryDynamicTable(tableName)
                  .insert([record]);
