...
export const logDeviceUsage = async (user: User, deviceType: 'mobile' | 'desktop') => {
  try {
    ...
    const { error } = await supabase
      .from('device_logs')
      .insert({
        // Match the field names in the database schema
...
export const clearDeviceLogs = async (): Promise<void> => {
  try {
    // To delete all rows, use a filter that matches everything
    const { error } = await supabase
      .from('device_logs')
      .delete()
      .neq('user_id', 'impossible-value-that-doesnt-exist'); // This will match all rows
